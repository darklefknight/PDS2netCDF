...
def calc_radarconst(zrg):
    ...
    C0 = (1e18 * 1024.0 * np.log(
        2.0) * pp.wvl ** 2 * pp.KBT0 * LossWG * pp.TxRecbandwidthMatchingLoss * pp.NfRec0 * pp.PRF0 * pp.H0 ** 2) / (
         np.pi ** 3 * pp.clight * pp.kwq * pp.AGain ** 2 * pp.ABeamWidth ** 2 * pp.PTAV0 * pp.tau0)
...
class srvi:
    def __init__(self, byte_array):
        self.values = {}
        self.elements = [("FrmCnt", "l"), ("Tm", "l"), ("TPow", "f"), ("NPw1", "f"), ("NPw2", "f"), ("CPw1", "f"),
                         ("CPw2", "f"), ("PS_Stat", "l"), ("RC_Err", "l"), ("TR_Err", "l"), ("GRS1", "l"),
                         ("GRS2", "l"), ("AzmPos", "f"), ("AzmVel", "f"), ("ElvPos", "f"), ("ElvVel", "f"),
                         ("NorthAngle", "f"), ("AzmSetPos", "f"), ("AzmSetVel", "f"), ("ElvSetPos", "f"),
                         ("ElvSetVel", "f")]
        # fill dictionary with unpacked values from byte array
        for i in range(0, len(self.elements)):
            self.values["%s" % self.elements[i][0]] = unpack('=%s' % self.elements[i][1], byte_array[i * 4:i * 4 + 4])[
                0]
    ...
    def __str__(self):
        ...
        content += "Npw1=%0.1f NPw2=%0.1f " % (
        10. * math.log10(self.values["NPw1"]), 10. * math.log10(self.values["NPw2"]))
        content += "CPw1=%0.1f CPw2=%0.1f " % (
        10. * math.log10(self.values["CPw1"]), 10. * math.log10(self.values["CPw2"]))
        ...
        while (byteCount < len(byteStructure)):
            ...
            if subsig[0:3] == "SNR" and ("SNR" or "Z" in product_list):
                ...
                if self.parameter.osc == 0:
                    (x, c) = self.getCoCxFFT(byteStructure[byteCount:byteCount + subsigSize],
                                             dtype=self.parameter.SPC_dtype).shape
