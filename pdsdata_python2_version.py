...
class ppar:
    def __init__(self, byte_array, magic=None):
        ...
        if ppar_structure[2]>=64:
          self.nfft = long(ppar_structure[2])
        else:
          self.nfft = 128L * 2L**ppar_structure[2]
...
class pdsdata:
    ...
    def evaluateByteStructure(self, byteStructure, product_list=[]):
        ...
        idx_fft = [i for i in range(int(self.parameter.nfft/2)-1,-1,-1)\
        +range(self.parameter.nfft-1,int(self.parameter.nfft/2)-1,-1)]
